...
`;function lt(t){if(!t||"unknown"===t||"Unknown"===t)return"—";const e=String(t).trim();if(!e)return"—";const i=e.match(/(\d{1,2}):(\d{2})(?::\d{2})?/);if(i)return`${i[1].padStart(2,"0")}:${i[2]}`;try{const t=new Date(e);return isNaN(t.getTime())?(console.warn("formatTime: unparseable value:",e),"—"):t.toLocaleTimeString("en-GB",{hour:"2-digit",minute:"2-digit",hour12:!1})}catch(t){return console.warn("formatTime: could not parse time value:",e,t),"—"}}function dt(t,e){if(!t||!e)return null;try{const i=new Date(t),n=new Date(e);if(!isNaN(i.getTime())&&!isNaN(n.getTime())){const t=Math.round((n-i)/6e4);return t>0?t:null}}catch(i){console.warn("calculateJourneyDuration: could not parse as dates:",t,e,i)}const i=String(t).match(/(\d{1,2}):(\d{2})/),n=String(e).match(/(\d{1,2}):(\d{2})/);if(i&&n){let t=60*parseInt(i[1],10)+parseInt(i[2],10),e=60*parseInt(n[1],10)+parseInt(n[2],10);e<t&&(e+=1440);const s=e-t;return s>0?s:null}return null}function ht(t){return!(!t||!t.expected_departure)&&(t.expected_departure!==t.scheduled_departure&&!/\d{1,2}:\d{2}/.test(t.expected_departure))}function pt(t){return t?t.is_cancelled?"cancelled":t.is_no_service?"no-service":t.delay_minutes>=10?"major-delay":t.delay_minutes>0||ht(t)?"minor-delay":"on-time":"unknown"}function ut(t,e=!0){return e&&t?t.is_cancelled?"❌":t.is_no_service?"⊗":t.delay_minutes>=10?"🔴":t.delay_minutes>0||ht(t)?"⚠️":"✓":""}function _t(t){return t?t.is_cancelled?t.cancel_reason||t.delay_reason||"":t.delay_reason||"":""}function mt(t){return t?t.is_cancelled?"Cancelled":t.is_no_service?"No service":t.delay_minutes>0?`Delayed ${t.delay_minutes} min${1!==t.delay_minutes?"s":""}`:ht(t)?"Delayed":"On time":"Unknown"}function gt(t){return t?t.is_cancelled?"Cancelled":t.is_no_service?"No service":t.expected_departure&&t.expected_departure!==t.scheduled_departure?`Exp ${lt(t.expected_departure)}`:"On time":"Unknown"}function ft(t,e=3){if(!t||0===t.length)return"";const i=t.slice(0,e),n=t.length-e;let s=i.join(", ");return n>0&&(s+=` +${n} more`),s}function vt(t,e="Platform"){return t?/bus/i.test(t)?t:`${e} ${t}`:`${e} —`}function yt(t){if(!t)return"";if(t.length<=12)return t;const e={London:"Ldn",Street:"St",Bridge:"Bdg",Junction:"Jn",Central:"Cen",International:"Intl",Station:"Stn",Road:"Rd",Cross:"X",Park:"Pk"};let i=t;for(const[t,n]of Object.entries(e))i=i.replace(new RegExp(t,"g"),n);return i.length>12&&(i=i.substring(0,11)+"…"),i}function bt(t,e){if(!t||0===t.length)return[];let i=[...t];return e.hide_on_time_trains&&(i=i.filter(t=>t.is_cancelled||t.is_no_service||t.delay_minutes>0||ht(t))),e.min_delay_to_show>0&&(i=i.filter(t=>t.is_cancelled||t.is_no_service||ht(t)||t.delay_minutes>=e.min_delay_to_show)),i}function $t(t){return t&&0!==t.length?[...t].sort((t,e)=>{const i=new Date(t.scheduled_departure).getTime(),n=new Date(e.scheduled_departure).getTime(),s=!isNaN(i),a=!isNaN(n);return s||a?s?a?i-n:-1:1:0}):[]}function wt(t){const e=new Map;for(const i of t){const t=i.destination&&String(i.destination).trim()||"Unknown";e.has(t)||e.set(t,[]),e.get(t).push(i)}return e}function xt(t){if(!t||0===t.length)return"normal";if(t.some(t=>t.is_cancelled))return"critical";const e=Math.max(...t.map(t=>t.delay_minutes||0));return e>=15?"severe":e>=10?"major":e>0?"minor":"normal"}function kt(t,e,i){return t?t.map((t,n)=>{const s=null!=t.train_number&&""!==t.train_number?String(t.train_number).toLowerCase().replace(/[^a-z0-9]/g,"_"):String(n+1),a=t.scheduled_departure,o=t.expected_departure,r=t.scheduled_arrival,c=t.estimated_arrival,l=/\d{1,2}:\d{2}/.test(String(o||"")),d=l?o:a,h=!l&&!!o&&o!==a&&!/^(on[\s-]?time|right\s*time)$/i.test(String(o||"").trim()),p=/\d{1,2}:\d{2}/.test(String(c||""))?c:r,u=null!=i?`leg_${i}_train_${s}`:`train_${s}`;return{...t,journey_duration:t.journey_duration||dt(d,p),journey_time_approx:t.journey_time_approx||h,train_id:`sensor.${e}_${u}`}}):[]}const Ct={"Missed Connection":"mdi:alert-octagon","Delayed Connection":"mdi:clock-alert","Tight Connection":"mdi:clock-alert-outline","Connection OK":"mdi:transit-connection-variant",Unknown:"mdi:help-circle-outline"};function Et(t){return Ct[t]||Ct.Unknown}const St={"Missed Connection":"connection-critical","Delayed Connection":"connection-major","Tight Connection":"connection-minor","Connection OK":"connection-normal",Unknown:"connection-normal"};function At(t){return St[t]||"connection-normal"}function Tt(t){const e=(t||"").toLowerCase();return e.includes("critical")||e.includes("severe")||e.includes("missed")?"status-critical":e.includes("major")||e.includes("delayed")?"status-major":e.includes("minor")||e.includes("tight")?"status-minor":"status-normal"}const Dt=new Set(["scheduled_departure","scheduled","departure","departure_time","std","aimed_departure_time","scheduled departure","expected_departure","expected","estimated","estimated_departure","etd","expected_arrival","expected departure","scheduled_arrival","sta","scheduled arrival","estimated_arrival","eta","estimated arrival","platform","operator","service_operator","is_cancelled","cancelled","is_no_service","no_service","delay_minutes","delay","minutes_late","delay minutes","delay_reason","reason","delay reason","cancel_reason","cancellation_reason","cancel reason","cancellation reason","calling_points","stops","calling_at","calling at","journey_duration","duration","journey_time_approx","service_type","type","train_number","train_id"]),jt=new Set(["attribution","icon","friendly_name","device_class","unit_of_measurement","supported_features","entity_picture","assumed_state","state_class","editable"]);function Mt(t){return t.replace(/[_-]+/g," ").trim().replace(/\b\w/g,t=>t.toUpperCase())}function Ot(t){if(null==t||""===t)return"—";if(Array.isArray(t))return t.length?t.join(", "):"—";if("boolean"==typeof t)return t?"Yes":"No";if("object"==typeof t)try{return JSON.stringify(t)}catch(e){return String(t)}return String(t)}customElements.define("my-rail-commute-card-editor",class extends ot{static get properties(){return{hass:{type:Object},_config:{type:Object}}}static get styles(){return a`
      .card-config {
        padding: 16px;
      }
    ...
    `}_renderGroupedTrains(){const t=wt(this._trains);return B`
      ${[...t.entries()].map(([t,e])=>{const i=this._servicesByDestination&&this._servicesByDestination[t],n=i?.status?i.status.toLowerCase().replace(/\s+/g,"-"):xt(e);return B`
    ...
    `}_renderCompactRow(t){const e=!0===this.config.show_journey_time,i=!1!==this.config.show_non_catchable_indicator&&!1===t.catchable;return B`
      ...
      >
        <span class="time">${lt(t.scheduled_departure)}</span>
        <span class="platform">${vt(t.platform,"Plat")}${e&&t.journey_duration?B` · ${t.journey_duration}m${t.journey_time_approx?"*":""}`:""}</span>
        <span class="status">
    ...
    `}_renderCompact(){const t=this._isMultiDestination&&!1!==this.config.group_by_destination,e=t=>this._renderCompactRow(t),i=t?(()=>{const t=wt(this._trains);return B`
            ${[...t.entries()].map(([t,i])=>{const n=this._servicesByDestination&&this._servicesByDestination[t],s=n?.status?n.status.toLowerCase().replace(/\s+/g,"-"):xt(i);return B`
                <div class="destination-group">
          ...
          <div class="board-table">
            ...
            ${this._trains.map(e=>B`
                ...
                >
                  ...
                  <span class="col-dest">
                    ${this._isMultiDestination?yt(e.destination||""):yt(this._destination||"")}
                  </span>
                  <span class="col-plat">
                    ${e.platform||"—"}
                  </span>
                  <span class="col-status">
                    ${gt(e)}${t&&e.journey_duration?` · ${e.journey_duration}m${e.journey_time_approx?"*":""}`:""}
                  </span>
                </div>
              `)}
          </div>
    ...
    `}_renderLegGroup(t,e,i,n){const s=Tt(t.overall_status),a="next-only"===n?t.services[0]?[t.services[0]]:[]:t.services;return B`
    ...
    `}_renderConnectionRow(t){const e=At(t.status),i=Et(t.status),n=!1!==this.config.show_connection_details;return B`
    ...
    `}_renderConnectionBoardRow(t){const e=At(t.status),i=Et(t.status),n=!1!==this.config.show_connection_details;return B`
            ...
            <div class="board-table">
              ...
              ${e.services.map(i=>B`
                ...
                >
                  ...
                  <span class="col-dest">
                    ${yt(i.destination||e.destination_name||"")}
                  </span>
                  <span class="col-plat">
                    ${i.platform||"—"}
                  </span>
                  <span class="col-status">
                    ${gt(i)}${t&&i.journey_duration?` · ${i.journey_duration}m${i.journey_time_approx?"*":""}`:""}
                  </span>
                </div>
              `)}
            </div>
