...
`;function dt(t){if(!t||"unknown"===t||"Unknown"===t)return"—";const e=String(t).trim();if(!e)return"—";const i=e.match(/(\d{1,2}):(\d{2})(?::\d{2})?/);if(i)return`${i[1].padStart(2,"0")}:${i[2]}`;try{const t=new Date(e);return isNaN(t.getTime())?e:t.toLocaleTimeString("en-GB",{hour:"2-digit",minute:"2-digit",hour12:!1})}catch(t){return console.warn("formatTime: could not parse time value:",e,t),e}}function lt(t){return!(!t||!t.expected_departure)&&(t.expected_departure!==t.scheduled_departure&&!/\d{1,2}:\d{2}/.test(t.expected_departure))}function ht(t){return t?t.is_cancelled?"cancelled":t.is_no_service?"no-service":t.delay_minutes>=10?"major-delay":t.delay_minutes>0||lt(t)?"minor-delay":"on-time":"unknown"}function ut(t,e=!0){return e&&t?t.is_cancelled?"❌":t.is_no_service?"⊗":t.delay_minutes>=10?"🔴":t.delay_minutes>0||lt(t)?"⚠️":"✓":""}function pt(t){return t?t.is_cancelled?"Cancelled":t.is_no_service?"No service":t.delay_minutes>0?`Delayed ${t.delay_minutes} min${1!==t.delay_minutes?"s":""}`:lt(t)?"Delayed":"On time":"Unknown"}customElements.define("my-rail-commute-card-editor",class extends ot{static get properties(){return{hass:{type:Object},_config:{type:Object}}}static get styles(){return r`
      .card-config {
        padding: 16px;
      }
      ...
      <ha-card class="${this.config.compact_height?"compact-height":""}">
        ...
        <div class="card-content compact">
          ${this._trains.map(e=>{const i=this._favorites.has(dt(e.scheduled_departure)),s=this._flagged.has(`${t}|${dt(e.scheduled_departure)}`);return L`
              <div
                class="train-row-compact ${ht(e)} ${i?"favourite":""} ${s?"flagged":""}"
              ...
              >
                ...
                <span class="status">
                  ${!1!==this.config.status_icons?L`<span class="status-icon">${ut(e)}</span>`:""}
        ...
        <div class="board-content">
          <div class="board-table">
            ...
            ${this._trains.map(i=>{const s=this._favorites.has(dt(i.scheduled_departure)),a=this._flagged.has(`${t}|${dt(i.scheduled_departure)}`);return L`
                <div
                  class="board-row ${ht(i)} ${s?"favourite":""} ${a?"flagged":""}"
