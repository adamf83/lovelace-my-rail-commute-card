...
`;function lt(t){if(!t||"unknown"===t||"Unknown"===t)return"—";const e=String(t).trim();if(!e)return"—";const i=e.match(/(\d{1,2}):(\d{2})(?::\d{2})?/);if(i)return`${i[1].padStart(2,"0")}:${i[2]}`;try{const t=new Date(e);return isNaN(t.getTime())?(console.warn("formatTime: unparseable value:",e),"—"):t.toLocaleTimeString("en-GB",{hour:"2-digit",minute:"2-digit",hour12:!1})}catch(t){return console.warn("formatTime: could not parse time value:",e,t),"—"}}function dt(t,e){if(!t||!e)return null;try{const i=new Date(t),n=new Date(e);if(!isNaN(i.getTime())&&!isNaN(n.getTime())){const t=Math.round((n-i)/6e4);return t>0?t:null}}catch(i){console.warn("calculateJourneyDuration: could not parse as dates:",t,e,i)}const i=String(t).match(/(\d{1,2}):(\d{2})/),n=String(e).match(/(\d{1,2}):(\d{2})/);if(i&&n){let t=60*parseInt(i[1],10)+parseInt(i[2],10),e=60*parseInt(n[1],10)+parseInt(n[2],10);e<t&&(e+=1440);const o=e-t;return o>0?o:null}return null}function ht(t){return!(!t||!t.expected_departure)&&(t.expected_departure!==t.scheduled_departure&&!/\d{1,2}:\d{2}/.test(t.expected_departure))}function pt(t){return t?t.is_cancelled?"cancelled":t.is_no_service?"no-service":t.delay_minutes>=10?"major-delay":t.delay_minutes>0||ht(t)?"minor-delay":"on-time":"unknown"}function ut(t,e=!0){return e&&t?t.is_cancelled?"❌":t.is_no_service?"⊗":t.delay_minutes>=10?"🔴":t.delay_minutes>0||ht(t)?"⚠️":"✓":""}function _t(t){return t?t.is_cancelled?"Cancelled":t.is_no_service?"No service":t.delay_minutes>0?`Delayed ${t.delay_minutes} min${1!==t.delay_minutes?"s":""}`:ht(t)?"Delayed":"On time":"Unknown"}function mt(t){return t?t.is_cancelled?"Cancelled":t.is_no_service?"No service":t.expected_departure&&t.expected_departure!==t.scheduled_departure?`Exp ${lt(t.expected_departure)}`:"On time":"Unknown"}function gt(t,e="Platform"){return t?/bus/i.test(t)?t:`${e} ${t}`:`${e} —`}function ft(t){if(!t)return"";if(t.length<=12)return t;const e={London:"Ldn",Street:"St",Bridge:"Bdg",Junction:"Jn",Central:"Cen",International:"Intl",Station:"Stn",Road:"Rd",Cross:"X",Park:"Pk"};let i=t;for(const[t,n]of Object.entries(e))i=i.replace(new RegExp(t,"g"),n);return i.length>12&&(i=i.substring(0,11)+"…"),i}function yt(t,e){if(!t||0===t.length)return[];let i=[...t];return e.hide_on_time_trains&&(i=i.filter(t=>t.is_cancelled||t.is_no_service||t.delay_minutes>0||ht(t))),e.min_delay_to_show>0&&(i=i.filter(t=>t.is_cancelled||t.is_no_service||ht(t)||t.delay_minutes>=e.min_delay_to_show)),i}function vt(t){return t&&0!==t.length?[...t].sort((t,e)=>{const i=new Date(t.scheduled_departure).getTime(),n=new Date(e.scheduled_departure).getTime(),o=!isNaN(i),s=!isNaN(n);return o||s?o?s?i-n:-1:1:0}):[]}function bt(t){const e=new Map;for(const i of t){const t=i.destination&&String(i.destination).trim()||"Unknown";e.has(t)||e.set(t,[]),e.get(t).push(i)}return e}function $t(t){if(!t||0===t.length)return"normal";if(t.some(t=>t.is_cancelled))return"critical";const e=Math.max(...t.map(t=>t.delay_minutes||0));return e>=15?"severe":e>=10?"major":e>0?"minor":"normal"}function wt(t,e,i){return t?t.map((t,n)=>{const o=null!=t.train_number&&""!==t.train_number?String(t.train_number).toLowerCase().replace(/[^a-z0-9]/g,"_"):String(n+1),s=t.scheduled_departure,r=t.expected_departure,a=t.scheduled_arrival,c=t.estimated_arrival,l=/\d{1,2}:\d{2}/.test(String(r||"")),d=l?r:s,h=!l&&!!r&&r!==s&&!/^(on[\s-]?time|right\s*time)$/i.test(String(r||"").trim()),p=/\d{1,2}:\d{2}/.test(String(c||""))?c:a,u=null!=i?`leg_${i}_train_${o}`:`train_${o}`;return{...t,journey_duration:t.journey_duration||dt(d,p),journey_time_approx:t.journey_time_approx||h,train_id:`sensor.${e}_${u}`}}):[]}const xt={"Missed Connection":"mdi:alert-octagon","Delayed Connection":"mdi:clock-alert","Tight Connection":"mdi:clock-alert-outline","Connection OK":"mdi:transit-connection-variant",Unknown:"mdi:help-circle-outline"};function Ct(t){return xt[t]||xt.Unknown}const kt={"Missed Connection":"connection-critical","Delayed Connection":"connection-major","Tight Connection":"connection-minor","Connection OK":"connection-normal",Unknown:"connection-normal"};function St(t){return kt[t]||"connection-normal"}function Et(t){const e=(t||"").toLowerCase();return e.includes("critical")||e.includes("severe")||e.includes("missed")?"status-critical":e.includes("major")||e.includes("delayed")?"status-major":e.includes("minor")||e.includes("tight")?"status-minor":"status-normal"}customElements.define("my-rail-commute-card-editor",class extends rt{static get properties(){return{hass:{type:Object},_config:{type:Object}}}static get styles(){return s`
      .card-config {
        padding: 16px;
      }
    ...
    `}_renderGroupedTrains(){const t=bt(this._trains);return B`
      ${[...t.entries()].map(([t,e])=>{const i=this._servicesByDestination&&this._servicesByDestination[t],n=i?.status?i.status.toLowerCase().replace(/\s+/g,"-"):$t(e);return B`
          <div class="destination-group">
            ${this._renderDestinationGroupHeader(t,n)}
            ${e.map(t=>this._renderTrainRow(t))}
          </div>
        `})}
    `}_renderDestinationGroupHeader(t,e){const i=Et(e);return B`
    ...
    `}_renderCompact(){const t=this._isMultiDestination&&!1!==this.config.group_by_destination,e=t=>this._renderCompactRow(t),i=t?(()=>{const t=bt(this._trains);return B`
            ${[...t.entries()].map(([t,i])=>{const n=this._servicesByDestination&&this._servicesByDestination[t],o=n?.status?n.status.toLowerCase().replace(/\s+/g,"-"):$t(i);return B`
                <div class="destination-group">
          ...
          <div class="board-table">
            ...
            ${this._trains.map(e=>B`
                ...
                >
                  ...
                  <span class="col-dest">
                    ${this._isMultiDestination?ft(e.destination||""):ft(this._destination||"")}
                  </span>
                  <span class="col-plat">
                    ${e.platform||"—"}
                  </span>
    ...
    `}_renderLegGroup(t,e,i,n){const o=Et(t.overall_status),s="next-only"===n?t.services[0]?[t.services[0]]:[]:t.services;return B`
    ...
    `}_renderConnectionBoardRow(t){const e=St(t.status),i=Ct(t.status),n=!1!==this.config.show_connection_details;return B`
            ...
            <div class="board-table">
              ...
              ${e.services.map(i=>B`
                ...
                >
                  ...
                  <span class="col-dest">
                    ${ft(i.destination||e.destination_name||"")}
                  </span>
                  <span class="col-plat">
                    ${i.platform||"—"}
                  </span>
