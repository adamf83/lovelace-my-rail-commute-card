...
`;function lt(t){if(!t||"unknown"===t||"Unknown"===t)return"—";const e=String(t).trim();if(!e)return"—";const i=e.match(/(\d{1,2}):(\d{2})(?::\d{2})?/);if(i)return`${i[1].padStart(2,"0")}:${i[2]}`;try{const t=new Date(e);return isNaN(t.getTime())?e:t.toLocaleTimeString("en-GB",{hour:"2-digit",minute:"2-digit",hour12:!1})}catch(t){return console.warn("formatTime: could not parse time value:",e,t),e}}function dt(t){return!(!t||!t.expected_departure)&&(t.expected_departure!==t.scheduled_departure&&!/\d{1,2}:\d{2}/.test(t.expected_departure))}function ht(t){return t?t.is_cancelled?"cancelled":t.is_no_service?"no-service":t.delay_minutes>=10?"major-delay":t.delay_minutes>0||dt(t)?"minor-delay":"on-time":"unknown"}function ut(t,e=!0){return e&&t?t.is_cancelled?"❌":t.is_no_service?"⊗":t.delay_minutes>=10?"🔴":t.delay_minutes>0||dt(t)?"⚠️":"✓":""}function pt(t){return t?t.is_cancelled?"Cancelled":t.is_no_service?"No service":t.delay_minutes>0?`Delayed ${t.delay_minutes} min${1!==t.delay_minutes?"s":""}`:dt(t)?"Delayed":"On time":"Unknown"}customElements.define("my-rail-commute-card-editor",class extends ot{static get properties(){return{hass:{type:Object},_config:{type:Object}}}static get styles(){return r`
    ...
    `}_getTrainsFromIndividualSensors(t,e){const i=(e||this.config.entity).replace("sensor.","").replace("_summary","").replace("_commute_summary",""),s=[`sensor.${i}_train_`,`sensor.${i}_train`,`sensor.${i.replace(/_/g,"-")}_train_`,`sensor.${i.replace(/_/g,"")}_train_`];let a=[];for(const e of s){const i=Object.keys(t.states).filter(t=>t.startsWith(e));if(i.length>0){a=i;break}}a.sort((t,e)=>parseInt(t.match(/train[_-]?(\d+)$/i)?.[1]||"0",10)-parseInt(e.match(/train[_-]?(\d+)$/i)?.[1]||"0",10));const r=a.map(e=>{const i=t.states[e];if(!i)return console.warn(`my-rail-commute-card: train sensor not found: ${e}`),null;let s=i.attributes.calling_points||i.attributes.stops||i.attributes.calling_at||i.attributes["Calling at"]||[];"string"==typeof s&&(s=s.split(",").map(t=>t.trim()).filter(t=>t));const a=i.attributes.scheduled_departure||i.attributes.scheduled||i.attributes.departure||i.attributes.departure_time||i.attributes.std||i.attributes.aimed_departure_time||i.attributes["Scheduled Departure"]||i.state;return{train_id:e,scheduled_departure:a,expected_departure:i.attributes.expected_departure||i.attributes.expected||i.attributes.estimated||i.attributes.estimated_departure||i.attributes.etd||i.attributes.expected_arrival||i.attributes["Expected Departure"]||a,platform:i.attributes.platform||i.attributes.Platform||"",operator:i.attributes.operator||i.attributes.service_operator||i.attributes.Operator||"",is_cancelled:i.attributes.is_cancelled||i.attributes.cancelled||"Cancelled"===i.state||"Canceled"===i.state||!1,is_no_service:i.attributes.is_no_service||i.attributes.no_service||"No service"===i.state||"No Service"===i.state||!1,delay_minutes:parseInt(i.attributes.delay_minutes||i.attributes.delay||i.attributes.minutes_late||i.attributes["Delay minutes"]||"0",10),delay_reason:i.attributes.delay_reason||i.attributes.reason||i.attributes["Delay reason"]||"",calling_points:s,journey_duration:i.attributes.journey_duration||i.attributes.duration||"",service_type:i.attributes.service_type||i.attributes.type||""}}).filter(t=>null!==t);return r}getCardSize(){const t=this.config.view||"full",e=this._trains?.length||0;switch(t){case"compact":return 1+Math.ceil(.5*e);case"next-only":return 3;default:return 2+e}}render(){if(!this.config.entity)return this._renderEmpty("No entity selected","Please select a rail commute summary sensor in the card configuration");if(this._loading)return this._renderLoading();if(t=this._hasDisruption,this.config.only_show_disrupted&&!t)return this._renderEmpty("No disruption detected","Trains will appear when there is disruption");var t;if(!this._trains||0===this._trains.length)return this._renderEmpty();switch(this.config.view||"full"){case"compact":return this._renderCompact();case"next-only":return this._renderNextOnly();case"board":return this._renderBoard();default:return this._renderFull()}}_renderHeader(){const t=!1!==this.config.show_header,e=!1!==this.config.show_route;if(!t)return"";const i=this.config.title||"Rail Commute";return L`
      <div class="card-header">
        <div class="header-content">
          ...
          `:""}
          ${this._favorites.size>0&&!1!==this.config.show_action_buttons?L`
            <button
              class="fav-filter-btn ${this._showFavoritesOnly?"active":""}"
              @click="${()=>{this._showFavoritesOnly=!this._showFavoritesOnly}}"
              title="${this._showFavoritesOnly?"Show all trains":"Show favourites only"}"
            >
    ...
    `}_renderFull(){const t=this.config.compact_height?"compact-height":"",e=this._showFavoritesOnly?this._trains.filter(t=>this._favorites.has(lt(t.scheduled_departure))):this._trains;return L`
      <ha-card class="${t}">
        ${this._renderHeader()}
    ...
    `}_renderTrainRow(t){const e=ht(t),i=!1!==this.config.status_icons?ut(t):"",s=!1!==this.config.show_platform,a=!1!==this.config.show_operator,r=!1!==this.config.show_delay_reason,o=!0===this.config.show_calling_points,n=!0===this.config.show_journey_time,c=this._favorites.has(lt(t.scheduled_departure)),l=(new Date).toISOString().split("T")[0],d=this._flagged.has(`${l}|${lt(t.scheduled_departure)}`);return L`
      <div
        class="train-row ${e} ${c?"favourite":""} ${d?"flagged":""}"
    ...
    `}_renderCompact(){const t=this._showFavoritesOnly?this._trains.filter(t=>this._favorites.has(lt(t.scheduled_departure))):this._trains,e=(new Date).toISOString().split("T")[0];return L`
      <ha-card class="${this.config.compact_height?"compact-height":""}">
        ${this._renderHeader()}
        ${this._renderDisruptionBanner()}

        <div class="card-content compact">
          ${t.map(t=>{const i=this._favorites.has(lt(t.scheduled_departure)),s=this._flagged.has(`${e}|${lt(t.scheduled_departure)}`);return L`
              <div
                class="train-row-compact ${ht(t)} ${i?"favourite":""} ${s?"flagged":""}"
                @click="${()=>this._handleTap(t)}"
                @touchstart="${this._handleTouchStart}"
                @touchend="${this._handleTouchEnd}"
                @touchmove="${this._handleTouchMove}"
              >
                ...
                <span class="status">
                  ${!1!==this.config.status_icons?L`<span class="status-icon">${ut(t)}</span>`:""}
                  ${t.delay_minutes>0?L`<span class="delay-text">+${t.delay_minutes}m</span>`:""}
                </span>
              </div>
            `})}
        </div>
    ...
    `}_renderNextOnly(){const t=this._trains[0];if(!t)return this._renderEmpty();const e=ht(t),i=!1!==this.config.status_icons?ut(t):"";return L`
      <ha-card class="${this.config.compact_height?"compact-height":""}">
        ${this._renderHeader()}
        ${this._renderDisruptionBanner()}
    ...
    `}_renderBoard(){const t=this._showFavoritesOnly?this._trains.filter(t=>this._favorites.has(lt(t.scheduled_departure))):this._trains,e=(new Date).toISOString().split("T")[0],i=!1!==this.config.show_action_buttons;return L`
        ...
        <div class="board-content">
          <div class="board-table">
            <div class="board-row board-header-row">
              ${i?L`<span class="col-actions"></span>`:""}
            ...
            ${t.map(t=>{const s=this._favorites.has(lt(t.scheduled_departure)),a=this._flagged.has(`${e}|${lt(t.scheduled_departure)}`);return L`
                <div
                  class="board-row ${ht(t)} ${s?"favourite":""} ${a?"flagged":""}"
                  @click="${()=>this._handleTap(t)}"
                  @touchstart="${this._handleTouchStart}"
                  @touchend="${this._handleTouchEnd}"
                  @touchmove="${this._handleTouchMove}"
